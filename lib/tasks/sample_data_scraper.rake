...
namespace :sample_data do
  ...
  task :scrape => :environment do
    ...
    begin
      ...
      FileUtils.mkdir_p(use_case_dir) unless Dir.exist?(use_case_dir)
      ...
      scene_headers.each do |scene_header|
        ...
        FileUtils.mkdir_p(scene_dir) unless Dir.exist?(scene_dir)
        ...
        if resource_list
          ...
          resource_list.css('li a').each do |resource_link|
            begin
              ...
              resource_page_url = resource_page_url.start_with?('http') ? resource_page_url : URI.join(ig_url, resource_page_url).to_s
              ...
              if json_tab_link
                ...
                json_tab_url = json_tab_url.start_with?('http') ? json_tab_url : URI.join(resource_page_url, json_tab_url).to_s

                puts "    Found JSON tab: #{json_tab_url}"

                # Navigate to the JSON tab
                json_page = Nokogiri::HTML(URI.open(json_tab_url))
                ...
                begin
                  ...
                  FileUtils.mkdir_p(resource_type_dir) unless Dir.exist?(resource_type_dir)
