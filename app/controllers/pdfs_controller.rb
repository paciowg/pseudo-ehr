...
class PdfsController < ApplicationController
  ...
  def show
    ...
    if params[:revoked] == 'true'
      ...
      send_data modified_pdf, filename: "document-#{params[:binary_id]}.pdf", type: 'application/pdf', disposition: 'inline'
    else
      send_data response.body, filename: "document-#{params[:binary_id]}.pdf", type: 'application/pdf', disposition: 'inline'
    end
  ensure
    temp_file.close
    temp_file.unlink
