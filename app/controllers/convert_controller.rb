...
class ConvertController < ApplicationController
  ...
  def index
		reply = @ehr_client.search(FHIR::QuestionnaireResponse)
		if good(reply)
			bundle = reply.resource
			@questionnaire_responses = bundle.entry
		else
			message = "Could not retrieve questionnaire responses from EHR: #{reply.code}"
  ...
  def show
    reply = @ehr_client.read(FHIR::QuestionnaireResponse, params[:id])
    if good(reply)
    	@sdc_questionnaire_response = reply.response
    else
    	message = "Error retrieving questionnaire response: #{reply.code}"
    	redirect_to root_path, flash: { error: message }
    end  	
  end
  ...
  def update
  	# Retrieve questionnaire response from FHIR server
    reply = @ehr_client.read(FHIR::QuestionnaireResponse, params[:id])
    if good(reply)
	    @sdc_questionnaire_response = reply.response
	    ...
	    @data_mgr_client.begin_transaction
	    ...
	    # Send PACIO resources as part of a FHIR transaction
	    @data_mgr_client.add_transaction_request('PUT', nil, questionnaire_response)
	    extract_data_from_questionnaire_response
	    reply = @data_mgr_client.end_transaction

	    if good(reply)
	    	message = "Wrote PACIO resources successfully to Data Manager"
	    	redirect_to root_path, notice: message
	    else
	    	message = "Error converting to PACIO resources: #{reply.code}"
	    	redirect_to root_path, flash: { error: message }
	    end
	  else
	  	message = "Could not retrieve questionnaire response from EHR: #{reply.code}"
	  	redirect_to root_path, flash: { error: message }
	  end
  ...
  def extract_data_from_questionnaire_response
    ...
    #@data_mgr_client.add_transaction_request('PUT', nil, bundled_observation)
  end
    ...
    if items[:item].present?
      items[:item].each do |item|
        if item[:item].present?
          ...
          if filter(item)
            puts 'yes'
            @data_mgr_client.add_transaction_request('PUT', nil, node_observation)
          end
       else
  ...
  def extract_leaf(fhir_questionnaire_response, bundled_observation, item)
    if item[:answer].present?
      ...
      @data_mgr_client.add_transaction_request('PUT', nil, fhir_observation)
  ...
  def setup_fhir_clients
    @data_mgr_client ||= FHIR::Client.new(HEALTH_DATA_MGR)
    @ehr_client ||= FHIR::Client.new(EHR_FHIR_SERVER)
  end
